...
DOCUMENTATION = '''
---
module: tetration_add_scopes_from_yaml
author: Matt Mullen
version_added: "1.0"
short_description: Import scopes to a Tetration cluster from a file in YAML format
description:
    - Connects to a specified Tetration cluster and imports all scopes to from a YAML file
...
options:
    ...
    source:
        description:
            - The YAML file to import the Scopes from
...
# ---------------------------------------------------------------------------
def ParentIDLookup(scopes,name):
    for scope in scopes:
        if scope['short_name'] == name:
            return scope['id']


def scope_upload(hostname,api_key,api_secret,source,validate_certs):
...
def main():
    ...
    code, response = scope_upload(module.params["hostname"],
